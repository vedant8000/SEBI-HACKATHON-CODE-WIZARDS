  ...
  if (!sections.length) {
    return (
      <Card className="p-10 text-center">
        ...
        <h3 className="text-base font-semibold text-slate-800">{t("dv.noDraft")}</h3>
        ...
        <button onClick={generateAll} disabled={!!busy}
          ...
          {busy === "all" ? t("dv.generating") : t("dv.generate")}
        </button>
        {!aiReady && <p className="text-[11px] text-slate-400 mt-2">{t("dv.noAiKey")}</p>}
      </Card>
    );
  }
    ...
    <div className="space-y-4">
      <div className="flex justify-end">
        <button onClick={generateAll} disabled={!!busy}
          ...
          <RefreshCw size={13} className={busy === "all" ? "animate-spin" : ""} /> {t("dv.regenAll")}
        </button>
      </div>
      {sections.map((s) => (
        <Card key={s.id} className="overflow-hidden">
          <div className="px-5 py-3 bg-slate-50 border-b border-slate-100 flex flex-wrap items-center gap-2">
            ...
            <div className="ml-auto flex items-center gap-1.5">
              <button onClick={() => regen(s.sectionName)} disabled={!!busy} title={t("dv.regenTitle")}
                className="p-1.5 text-slate-500 hover:text-blue-600 hover:bg-blue-50 rounded">
                {busy === s.sectionName ? <Loader2 size={14} className="animate-spin" /> : <RefreshCw size={14} />}
              </button>
              <button onClick={() => { setEditing(editing === s.id ? null : s.id); setEditText(s.generatedText); }} title={t("dv.editTitle")}
                className="p-1.5 text-slate-500 hover:text-blue-600 hover:bg-blue-50 rounded"><Pencil size={14} /></button>
              <button onClick={() => setCommentFor(commentFor === s.id ? null : s.id)} title={t("dv.commentTitle")}
                className="p-1.5 text-slate-500 hover:text-blue-600 hover:bg-blue-50 rounded"><MessageSquarePlus size={14} /></button>
              <button onClick={() => review(s.id, "mark-review")} disabled={!!busy} title={t("dv.sendReviewTitle")}
                ...
                <FileCheck size={13} /> {t("dv.markReview")}
              </button>
            </div>
          </div>
          <div className="px-5 py-4 grid gap-5 xl:grid-cols-[1fr_290px]">
              ...
              <div className="mt-3">
                ...
                <div className="flex gap-2 mt-2">
                  <button onClick={() => { patch(s.id, { text: editText, status: "Promoter Reviewed" }); setEditing(null); }}
                    className="px-3 py-1.5 bg-blue-600 text-white text-xs rounded-lg">{t("dv.saveEdits")}</button>
                  <button onClick={() => setEditing(null)} className="px-3 py-1.5 border border-slate-300 text-xs rounded-lg">{t("dv.cancel")}</button>
                </div>
              </div>
            ) : (
            ...
            {s.sources.length > 0 && (
              <div className="mt-4 bg-blue-50/70 border-l-2 border-blue-500 rounded-r-lg px-3 py-2">
                <div className="text-[11px] font-semibold text-blue-800 mb-1">{t("dv.sourceEvidence")}</div>
            ...
            {s.missingData.length > 0 && (
              <div className="mt-2 bg-amber-50 border-l-2 border-amber-500 rounded-r-lg px-3 py-2">
                <div className="text-[11px] font-semibold text-amber-800 mb-0.5">{t("dv.sourceMissing")}</div>
            ...
            {commentFor === s.id && (
              <div className="mt-3 flex gap-2">
                <input className="flex-1 px-3 py-1.5 text-sm border border-slate-300 rounded-lg"
                  placeholder={t("dv.notePh")} value={commentText} onChange={(e) => setCommentText(e.target.value)} />
                <button onClick={() => { review(s.id, "comment", commentText); setCommentText(""); setCommentFor(null); }}
                  className="px-3 py-1.5 bg-slate-800 text-white text-xs rounded-lg">{t("dv.post")}</button>
              </div>
            )}
            {s.comments.length > 0 && (
...
function SectionInsightPanel({ m, status }: { m?: SectionMeta; status: string }) {
  ...
  return (
    <aside className="space-y-3 xl:border-l xl:border-slate-100 xl:pl-5">
      <div>
        <div className="text-[11px] font-semibold text-slate-500 uppercase tracking-wide mb-1">{t("dv.inPlainEnglish")}</div>
        <p className="text-[12.5px] text-slate-600 leading-relaxed">
          {t("dv.peBody", { purpose: m.purpose.charAt(0).toLowerCase() + m.purpose.slice(1), pct: m.coveragePct })}
          {status === "AI Drafted" ? t("dv.peDrafted") : status === "Approved" ? t("dv.peApproved") : t("dv.pePeriod")}
          {m.professionalReviewRequired && t("dv.peProf")}
        </p>
      </div>
      {m.missingFacts.length > 0 && (
        <div>
          <div className="text-[11px] font-semibold text-amber-700 uppercase tracking-wide mb-1">{t("dv.stillMissing")}</div>
      ...
      {m.questions.length > 0 && (
        <div>
          <div className="text-[11px] font-semibold text-blue-700 uppercase tracking-wide mb-1">{t("dv.likelyQuestions")}</div>
